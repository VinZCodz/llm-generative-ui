...
export const ChatBubble = memo(({ message }: { message: StreamMessage }) => {
  ...
  return (
    <motion.div
      ...
      className={cn("flex w-full mb-5", isIncoming ? "justify-start" : "justify-end")}
    >
      <div className={cn("flex max-w-[85%] md:max-w-[75%] gap-3", isIncoming ? "flex-row" : "flex-row-reverse")}>
        ...
        {(isConversation) && payload.text.length > 0 && (
          <div className={cn(
            "w-8 h-8 rounded-full flex items-center justify-center shrink-0 mt-2 border",
        ...
        <div className="space-y-2">
          ...
          {type === "toolCall:start" && (
            <div className="flex flex-col p-3 rounded-xl ml-11 bg-purple-900/10 border border-purple-500/30">
          ...
          {type === 'tool' && (
            <div className="flex flex-col p-3 rounded-xl ml-11 bg-green-900/10 border border-green-500/30">
              <div className="flex items-center gap-1 mb-1">
              ...
              <div className="items-center gap-1">
                {Array.isArray(payload.result) ?
                  (
                    <div className="flex items-center px-2 py-1 rounded-2xl bg-green-500/20 border border-green-400/50">
          ...
          {type === 'tool' && payload.name.toLowerCase().includes('chart') && (
            <div className="flex flex-col p-3 rounded-xl ml-11 mt-6 bg-black-900 border border-zinc-800 text-zinc-200">
              <div className="flex items-center gap-2 mb-1">
                <ExpenseChart chartData={payload.result.data} />
              </div>
            </div>
          )}
        </div>
